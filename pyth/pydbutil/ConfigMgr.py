        ...
        for filename in match_dict.keys():
            ...
            for m in match_dict[filename]:
                ...
                dbset_match = self.dbset.get_profile_by_attribs(
                        dict(boxname=m_boxname, dbname=m_dbname, app=app, env=env))
                print '  {} matched {} from the dbset.'.format(m, dbset_match)
                if not dbset_match:
                    print '    *Similar dbset profile is {}'.format(
                           self.dbset.get_profile_by_attribs(dict(dbname=m_dbname, app=app, env=env)))
