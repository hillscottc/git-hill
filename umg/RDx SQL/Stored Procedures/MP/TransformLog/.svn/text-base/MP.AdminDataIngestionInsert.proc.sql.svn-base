CREATE PROCEDURE MP.AdminDataIngestionInsert(@sessionID bigint)
AS
...
FROM
( 
	SELECT tlt.SessionID, tlt.WorkflowCode, tlt.Rows FROM MP.TransformLogTask tlt where tlt.SessionID = @sessionID		
) AS t
PIVOT 
( 
