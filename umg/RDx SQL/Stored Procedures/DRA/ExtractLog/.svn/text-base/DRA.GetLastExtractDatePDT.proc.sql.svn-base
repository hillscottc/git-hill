CREATE PROCEDURE DRA.GetLastExtractDatePDT(@date datetime output)
AS
	...
	from 
		DRA.ExtractLog
	where 
		ID = (select max(ID) from DRA.ExtractLog where [Status] = 'C')
	
	-- DRA is PDT time zone. We are CDT time zone. Convert to PDT before comparing and returning data.
