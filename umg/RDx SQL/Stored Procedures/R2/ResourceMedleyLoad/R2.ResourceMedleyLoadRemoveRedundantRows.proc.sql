-- ============================================
-- Description:	Clears duplicate rows from R2.LoadResourceMedley. 
-- =============================================
CREATE PROCEDURE [R2].[ResourceMedleyLoadRemoveRedundantRows] 
AS
BEGIN

	-- Clean records with the same RESOURCE_RESOURCE_LINK_ID and different change datetime.
	delete R2.LoadResourceMedley
	from 
		R2.LoadResourceMedley r
		inner join
		(
			select RESOURCE_RESOURCE_LINK_ID, MAX_CHANGE_DATE_TIME = max(CHANGE_DATE_TIME) 
			from R2.LoadResourceMedley 
