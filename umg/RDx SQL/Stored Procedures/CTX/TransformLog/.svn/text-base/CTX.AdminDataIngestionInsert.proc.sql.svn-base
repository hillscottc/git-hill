CREATE PROCEDURE CTX.AdminDataIngestionInsert(@sessionID bigint)
AS
...
FROM
( 
	SELECT tlt.SessionID, tlt.WorkflowCode, tlt.Rows FROM CTX.TransformLogTask tlt where tlt.SessionID = @sessionID		
) AS t
PIVOT 
( 
